...
ScavTrap::~ScavTrap(void){
	std::cout << BGREEN "ScavTrap " << RED << _name << " has been destroyed" << RESET << std::endl;
};
...
void ScavTrap::attack(const std::string& target){
	if(this->_hitPoints < 1){
		std::cout << BGREEN << "ScavTrap " << RESET << this->getName() << YELLOW <<" is dead"<< RESET << std::endl;
		return ;
	}
	if(this->_energyPoints == 0){
		std::cout << BGREEN << "ScavTrap " << RESET << this->getName() << YELLOW <<" tried attacking, but no energy left"<< RESET << std::endl;
		return ;
	}
	std::cout << BGREEN << "ScavTrap " << RESET << this->getName() <<" attacks " << target
	<< " causing " << RED << this->getAttack() << RESET << " points of damage !" << std::endl;
	--this->_energyPoints;
};

void ScavTrap::guardGate(void){
	std::cout << BGREEN << "ScavTrap " << RESET << this->getName() 
